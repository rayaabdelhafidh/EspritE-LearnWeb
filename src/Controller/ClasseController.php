...
public function Tri(Request $request, ClasseRepository $repository, PaginatorInterface $paginator): Response
{
    ...
    {
        foreach($userRepository->findByRole("ROLE_ENSEIGNANT") as $enseignant ){
            $listeEnseignant = [];
            if($enseignant->getIdClasse() == null )
            {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
           $reqIdEnseignant= $request->get('users');
            $entityManager->persist($classe);
            $entityManager->flush();
            flash()->addSuccess('classe ajoutée avec succés');
            $listeUsers=[];
            $listeUsers=$classe->getUsers();
            $listeUsers[] = $userRepository->find($reqIdEnseignant);
    ...
    public function edit(Request $request, Classe $classe, EntityManagerInterface $entityManager,UserRepository $userRepository): Response
    {
        $form = $this->createForm(ClasseType::class, $classe);
        $form->handleRequest($request);
        foreach($userRepository->findByRole("ROLE_ENSEIGNANT") as $enseignant ){
            $listeEnseignant = [];
            if($enseignant->getIdClasse() == null || $enseignant->getIdClasse() == $classe)
            {
                $listeEnseignant[]= $enseignant;
            }
        }
